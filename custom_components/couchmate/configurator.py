...
class CouchMateConfiguratorSaveView(HomeAssistantView):
    ...
    async def post(self, request):
        ...
        humidities = {
            str(area_id): str(entity_id)
            for area_id, entity_id in dict(payload.get("room_humidities", {})).items()
            if area_id and entity_id
        }
        explicit_entities = list(dict.fromkeys(payload.get("entities", [])))

        # An explicit function selection must always win over broad area/device
        # selections. Earlier configurator builds automatically marked the parent
        # device and area when a single entity was checked. That caused every
        # sibling entity (for example battery or diagnostics) to be exposed too.
        entity_registry = er.async_get(hass)
        device_registry = dr.async_get(hass)
        explicit_device_ids: set[str] = set()
        explicit_area_ids: set[str] = set(temperatures) | set(humidities)
        for entity_id in explicit_entities:
            entry = entity_registry.async_get(entity_id)
            if entry is None:
                continue
            if entry.device_id:
                explicit_device_ids.add(entry.device_id)
                device = device_registry.async_get(entry.device_id)
                if device and device.area_id:
                    explicit_area_ids.add(device.area_id)
            if entry.area_id:
                explicit_area_ids.add(entry.area_id)

        selected_devices = [
            device_id for device_id in dict.fromkeys(payload.get("devices", []))
            if device_id not in explicit_device_ids
        ]
        selected_areas = [
            area_id for area_id in dict.fromkeys(payload.get("areas", []))
            if area_id not in explicit_area_ids
        ]
