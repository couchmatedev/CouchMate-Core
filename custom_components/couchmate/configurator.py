...
fetch('/api/couchmate/configurator/data').then(r=>r.json()).then(j=>{data=j;for(const a of data.areas){if(a.selected)selected.areas.add(a.id);for(const d of a.devices){if(d.selected)selected.devices.add(d.id);for(const e of d.entities){if(e.selected)selected.entities.add(e.entity_id);if(e.excluded)selected.excluded.add(e.entity_id)}}}renderAreas()});
